...
describe(findSiteCandidates.name, () => {
  ...
  test("finds a site with same domain", () => {
    expect(
      findSiteCandidates(
        [aDefaultSiteConfig],
        "http://example.com/map?zoom=1&lat=2&lon=3"
      )
    ).toEqual([aDefaultSiteConfig.id]);
  });
  test("finds a site with domainRegexp", () => {
    const siteConfigWithAdjustment: SiteConfiguration = {
      ...aDefaultSiteConfig,
      defaultConfiguration: {
        ...aDefaultSiteConfig.defaultConfiguration!,
        domainRegexp: /example\.com\.[a-z]{2}$/,
      },
    };
    expect(
      findSiteCandidates(
        [siteConfigWithAdjustment],
        "http://example.com.br/map?zoom=1&lat=2&lon=3"
      )
    ).toEqual([aDefaultSiteConfig.id]);
  });
  ...
  test("get user url template that that includes domain", () => {
    const configUrlPat: SiteConfiguration = {
      ...
      customPattern: {
        tag: "user-v1",
        url: "https://www.example.com/map?zoom={zoom}&lat={latitude}&lon={longitude}",
      },
    };
    expect(
      findSiteCandidates(
  ...
  test("known google domains properly match current google configuration", () => {
    knownGoogleDomains.forEach((domain) => {
      ...
      const googleConfiguration = {
        id: "googlemaps",
        isEnabled: true,
        defaultConfiguration: Sites["googlemaps"],
      };
      expect(findSiteCandidates([googleConfiguration], inputUrl)).toEqual([
        "googlemaps",
      ]);
    });
  });
...
describe(pickWinningCandidate.name, () => {
    ...
    const inputAttributes = [
      {
        ...
        additionalAttributes: { zoom: "1", lat: "2", lon: "3" },
      },
    ];
    const expectedOutput = {
      siteId: aDefaultSiteConfig.id,
      attributes: inputAttributes[0].additionalAttributes,
    };
    expect(
      pickWinningCandidate(
        [aDefaultSiteConfig],
  ...
  const userUrlTemplateTests: {
    exampleUrl: string;
    urlTemplate: string;
    expectedAttrs: Partial<Record<OsmAttribute, string>>;
  }[] = [
    {
      exampleUrl:
        "https://geohack.toolforge.org/geohack.php?params=23.00_N_24.43_E",
      urlTemplate:
    ...
    {
      exampleUrl:
        "https://hiking.waymarkedtrails.org/#route?id=2966504&map=3!23.0!24.1",
      urlTemplate:
        "https://hiking.waymarkedtrails.org/#route?id={osm_relation_id}",
      expectedAttrs: { relationId: "2966504" },
    },
    {
      exampleUrl:
        "https://hiking.waymarkedtrails.org/#route?id=10534456&map=14!58.4593!11.4308",
      urlTemplate:
        "https://hiking.waymarkedtrails.org/#route?id={osm_relation_id}&map={zoom}!{latitude}!{longitude}",
      expectedAttrs: {
        relationId: "10534456",
        zoom: "14",
        lat: "58.4593",
  ...
  userUrlTemplateTests.forEach((testParams) => {
    test(`get parameters from url ${testParams.exampleUrl}`, () => {
      const inputConfig: SiteConfiguration = {
        id: "user-pattern",
        isEnabled: true,
        customPattern: { tag: "user-v1", url: testParams.urlTemplate },
      };
      const expectedOutput = {
        siteId: inputConfig.id,
        attributes: testParams.expectedAttrs,
      };
      expect(
        pickWinningCandidate(
          [inputConfig],
          [{ siteId: inputConfig.id }],
          testParams.exampleUrl
        )
      ).toEqual(expectedOutput);
    });
  });
  test("get osmchangetiles from weird url", () => {
    ...
    const inputConfig = {
      isEnabled: true,
      id: expectedSiteId,
      defaultConfiguration: Sites[expectedSiteId],
    };
    const inputUrl =
      "https://resultmaps.neis-one.org/osm-change-tiles?quadkey=1202200110303320#16/48.7537/2.3536";
    const expectedOutput = {
      siteId: expectedSiteId,
      attributes: { zoom: "16", lat: "48.7537", lon: "2.3536" },
    };
    expect(
      pickWinningCandidate(
        [inputConfig],
  ...
  test("get google from weird url", () => {
    ...
    const expectedOutput = {
      siteId: expectedSiteId,
      attributes: { zoom: "17", lat: "24.18169", lon: "120.86617" },
    };
    expect(
      pickWinningCandidate(
        [inputConfig],
  ...
  test(`get parameters from a url and not the permalink`, () => {
    const inputConfig: SiteConfiguration = {
      id: "url-not-permalink",
      isEnabled: true,
      customPattern: {
        tag: "user-v1",
        url: "https://wiki.openstreetmap.org/wiki/Key:{osm_tag_key}",
      },
    };
    const pageInput = [
      {
        siteId: inputConfig.id,
        permalink:
          "https://wiki.openstreetmap.org/w/index.php?title=Key:name&oldid=2013483",
      },
    ];
    const expectedOutput = {
      siteId: inputConfig.id,
  ...
  test(`recognize parameters from 'osmose'`, () => {
    ...
    const inputConfig: SiteConfiguration = {
      id,
      isEnabled: true,
      defaultConfiguration: Sites[id],
    };
    const expectedOutputAttributes = {
      zoom: "18",
      lat: "48.439383",
      lon: "-4.416006",
    };
    ...
    ).toEqual(expectedOutputAttributes);
  });
  ...
  describe("zoom", () => {
    ...
    test("with zoomAdjustment=12", () => {
      const inputAttributes = [
        {
          siteId: aDefaultSiteConfig.id,
          additionalAttributes: { zoom: "5", lat: "6", lon: "7" },
        },
      ];
      const siteConfigWithAdjustment: SiteConfiguration = {
        ...aDefaultSiteConfig,
...
describe(getRelevantSites.name, () => {
  ...
  test("applies zoom,lat,lon to a site", () => {
    const expectedOutput = [
      { id: aDefaultSiteConfig.id, url: "https://example.com/#map=5/6/7" },
    ];
    expect(
      getRelevantSites([aDefaultSiteConfig], "", zll567_attributes)
    ).toEqual(expectedOutput);
  });
  test("applies changeset id to osm site", () => {
    const basicPattern: SiteConfiguration = {
      ...
      isEnabled: true,
      customName: "a-name",
      defaultConfiguration: Sites.openstreetmap,
    };
    const expectedOutput = [
      {
        id: basicPattern.id,
        customName: basicPattern.customName,
        url: "https://www.openstreetmap.org/changeset/83729",
      },
    ];
    expect(
      getRelevantSites([basicPattern], "", { changesetId: "83729" })
    ).toEqual(expectedOutput);
  });
    ...
    const expectedOutput = [
      {
        id: basicPattern.id,
        customName: basicPattern.customName,
        url: "https://www.openstreetmap.org/#map=5/6/7",
      },
    ];
    expect(getRelevantSites([basicPattern], "", zll567_attributes)).toEqual(
      expectedOutput
    );
  });
  test("user-v1 pattern is case-sensitive", () => {
    const basicPattern: SiteConfiguration = {
      ...
      customPattern: {
        tag: "user-v1",
        url: "https://www.waze.com/pt-BR/editor?env=row&lon={longitude}&lat={latitude}&zoom=7",
      },
    };
    const expectedOutput = [
      {
        id: basicPattern.id,
        customName: basicPattern.customName,
        url: "https://www.waze.com/pt-BR/editor?env=row&lon=7&lat=6&zoom=7",
      },
    ];
    expect(getRelevantSites([basicPattern], "", zll567_attributes)).toEqual(
      expectedOutput
  ...
  describe("zoom", () => {
    test("with zoomAdjustment=1", () => {
      const expectedOutput = [
        { id: aDefaultSiteConfig.id, url: "https://example.com/#map=4/6/7" },
      ];
      const siteConfigWithAdjustment: SiteConfiguration = {
        ...aDefaultSiteConfig,
        defaultConfiguration: {
      ...
      expect(
        getRelevantSites([siteConfigWithAdjustment], "", zll567_attributes)
      ).toEqual(expectedOutput);
    });
    test("with zoomAdjustment=3", () => {
      const expectedOutput = [
        { id: aDefaultSiteConfig.id, url: "https://example.com/#map=2/6/7" },
      ];
      const siteConfigWithAdjustment: SiteConfiguration = {
        ...aDefaultSiteConfig,
        defaultConfiguration: {
    ...
    test("with maxZoom smaller than zoom", () => {
      const expectedOutput = [
        { id: aDefaultSiteConfig.id, url: "https://example.com/#map=3/6/7" },
      ];
      const siteConfigWithAdjustment: SiteConfiguration = {
        ...aDefaultSiteConfig,
        defaultConfiguration: {
    ...
    test("with maxZoom greater than a decimal zoom", () => {
      const expectedOutput = [
        {
          id: aDefaultSiteConfig.id,
          url: "https://example.com/#map=9.2109/6/7",
        },
      ];
      const siteConfigWithAdjustment: SiteConfiguration = {
        ...aDefaultSiteConfig,
      ...
      expect(getRelevantSites([siteConfigWithAdjustment], "", input)).toEqual(
        expectedOutput
      );
    });
