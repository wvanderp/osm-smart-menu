...
export function pickWinningCandidate(
  ...
  if (site) {
    ...
    [detectedPattern, extractedAttributes] = detectAndExtractAttributesFromUrl(
      head.permalink || currentTabUrl
    );
  }
  const allExtractedAttributes = Object.assign(
    extractedAttributes,
    head.additionalAttributes
  );
...
function withAdjustedZoom(
  config: SiteConfiguration | undefined,
  extractedAttributes: Record<string, string>
): Record<string, string> {
  if (
    extractedAttributes.zoom &&
    config &&
    config.defaultConfiguration &&
    config.defaultConfiguration.zoomAdjustment
  ) {
...
export function getRelevantSites(
  ...
  return sitesConfiguration
    .map(function (site): SiteLink | undefined {
      if (site.id == currentSiteId || !site.isEnabled) return undefined;

      const { id, customName } = site;
      if (site.defaultConfiguration) {
        const chosenOption = site.defaultConfiguration.paramOpts.find(function (
          paramOpt
        ) {
          const [orderedParameters, unorderedParameters] =
            extractParametersFromParamOpt(paramOpt);
          const necessaryParameters =
            orderedParameters.concat(unorderedParameters);
          return necessaryParameters.every(
            (param) => retrievedAttributes[param] !== undefined
          );
        });
        ...
        if (retrievedAttributes.zoom) {
          attributes = {
            ...retrievedAttributes,
            zoom: reviewZoom(
              site.defaultConfiguration,
              retrievedAttributes.zoom
            ),
          };
        }
        ...
        if (chosenOption) {
          ...
          const protocol = site.defaultConfiguration.httpOnly
            ? "http"
            : "https";
          return {
            id,
        ...
        if (site.customPattern) {
          const url = applyParametersToUrlPattern(
            site.customPattern,
            retrievedAttributes
          );
          if (url) {
...
function applyParametersToUrlPattern(
  ...
  if (urlPattern.tag === "user-v1") {
    ...
    const hasNecessaryParameters = parameters.every(
      (key) => userUrlParametersMap[key] in retrievedAttributes
    );
    ...
    parameters.forEach(function (key: string) {
      url = url.replace(
        "{" + key + "}",
        retrievedAttributes[userUrlParametersMap[key]]
      );
    });
    return url;
  ...
  if (urlPattern.tag === "hash-2") {
    const matchArray = urlObj.hash.match(
      /(#[a-z=]*)([0-9.]+)(\/)([0-9.-]+)(\/)([0-9.-]+)(.*)/
    );
    ...
    Object.entries(urlPattern.querystringSubst).forEach(
      ([osmAttribute, querystringParameter]): void => {
        urlObj.searchParams.set(
          querystringParameter,
          retrievedAttributes[osmAttribute]
        );
      }
    );
    return urlObj.toString();
    ...
    Object.entries(urlPattern.hashParametersSubst).forEach(
      ([osmAttribute, hashParameter]): void => {
        auxUrl.searchParams.set(
          hashParameter,
          retrievedAttributes[osmAttribute]
        );
      }
...
export type UrlPattern =
  | SimpleQuerystringPattern
  | HashWithNamedParametersPattern
  | OsmLikePattern
  | UserUrlPattern;

type SimpleQuerystringPattern = {
...
type HashWithNamedParametersPattern = {
  // example https://www.osmhydrant.org/en/#zoom=14&lat=48.20168&lon=16.48777
  tag: "hash-1";
...
function detectAndExtractAttributesFromUrl(
  url: string
): [UrlPattern | undefined, Record<string, string>] {
...
function extractAttributesFromUrlPattern(
  url: URL,
  urlPattern: UrlPattern
): Record<string, string> {
  if (urlPattern.tag === "hash-2") {
...
function extractAttributesFromUrl(
  url: string,
  siteConfig: SiteConfiguration
): Record<string, string> {
  if (siteConfig.customPattern) {
    return extractAttributesFromUrlPattern(
      new URL(url),
    ...
    for (let i = 0; i < config.paramOpts.length; i++) {
      let extractedAttributes: Record<string, string> = {};
      const [orderedParameters] = extractParametersFromParamOpt(
        config.paramOpts[i]
      );
      ...
      if (orderedMatch) {
        ...
        const matchesUnordered = Object.entries(unorderedParametersMap).every(
...
function extractOsmPatternExtraction(
  url: URL
...
function extractParametersFromParamOpt(
  paramOpt: ParamOpt
): [string[], string[]] {
  return [
...
function applyParametersToUrl(
  ...
  if (option.unordered) {
    const urlQueryParameters = Object.entries(option.unordered).map(function ([
      key,
      value,
    ]) {
